...
FrequencyAxisResolution, HOP, Band, 1, 1);
    mae_vec(i) = mae;
...
WindowLength, WindowBandwidth, HighFrequencyLimit, LowFrequencyLimit, ...
FrequencyAxisResolution, HOP, Band, 1, 10);
    mae_vec(i) = mae;
    recon_mat(:, i) = real(recon)';
end

truth = signal.pleth.y;
truth = truth - mean(truth);
time = (1:length(truth))' / Hz;

figure;
plot(time, truth - mean(truth));
hold on;
plot(time(1:HOP:end), recon,'r');
